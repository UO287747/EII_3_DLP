...
expression returns [Expression ast]:
            ...
            | '('type')' e1=expression { $ast = new Cast($e1.ast.getLine(), $e1.ast.getColumn(), $e1.ast, $type.ast); }
...
statement returns [List<Statement> ast = new ArrayList<Statement>()]:
            e='print' expressions ';'{
                for (Expression expression: $expressions.ast) {
                    $ast.add(new Print($e.getLine(), expression.getColumn(), expression));
                }
            }
            | e='input' expressions ';'{
                for (Expression expression: $expressions.ast) {
                    $ast.add(new Input($e.getLine(), expression.getColumn(), expression));
                }
            }
            | e1=expression OP='=' e2=expression ';' { $ast.add( new Assignment($OP.getLine(), $OP.getCharPositionInLine()+1, $e1.ast, $e2.ast)); }
            | e='if' expression ':' loop_body ('else' ':' loop_body)?
...
functionExpression returns [Expression ast] locals [ List<Expression> list = new ArrayList<Expression>() ]:
        ID '(' (expressions{ $list.addAll($expressions.ast); })? ')' { $ast = new FuncCall($ID.getLine(), $ID.getCharPositionInLine()+1,
        $list, new Variable($ID.getLine(), $ID.getCharPositionInLine()+1, $ID.text)); };
...
struct_field returns [ List<RecordField> ast = new ArrayList<RecordField>() ]:
        (ID ':' type ';' {
            $ast.add( new RecordField($ID.getLine(), $ID.getCharPositionInLine()+1, $ID.text, $type.ast));
        })+;
